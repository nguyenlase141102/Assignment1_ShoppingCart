 ...
 * @author ANH NGUYEN
 */
public class SelectedFoodServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            ...
            food = FoodDAO.getFoodByIDs(id);
            cart.addCart(food);

            session.setAttribute("recordFoodToDelete",cart);
            
            request.setAttribute("textMess", name + "to be selected");
            RequestDispatcher dis = request.getRequestDispatcher("LoadHomePage");
            dis.forward(request, response);
        }catch(SQLException ex){
            log("SelectedFoodServlet: "+ex.getMessage());
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(SelectedFoodServlet.class.getName()).log(Level.SEVERE, null, ex);
        } catch (NamingException ex) {
            Logger.getLogger(SelectedFoodServlet.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(SelectedFoodServlet.class.getName()).log(Level.SEVERE, null, ex);
